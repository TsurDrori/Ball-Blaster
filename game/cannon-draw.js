// ── ציור תותח — הרחבת prototype של Cannon ───────────────────────────────────
// נטען אחרי cannon.js

Cannon.prototype._armorColors = function() {
...
Cannon.prototype.draw = function(ctx) {
    ...
    if (gameState.hasRunUpgrade('magnetic')) {
        ...
        ctx.fillStyle = mg;
        ctx.beginPath(); ctx.arc(this.x, this.y, mr, 0, Math.PI * 2); ctx.fill();
    ...
    if (gameState.shieldTimer > 0) {
        ...
        ctx.fillStyle = sg;
        ctx.beginPath(); ctx.arc(this.x, this.y, sr, 0, Math.PI * 2); ctx.fill();
    ...
    if (ac.glow) {
        ...
        glow.addColorStop(0, ac.glow); glow.addColorStop(1, 'rgba(0,0,0,0)');
        ctx.fillStyle = glow;
        ctx.beginPath(); ctx.arc(this.x, this.y, 72, 0, Math.PI * 2); ctx.fill();
    }
    ...
    baseGrad.addColorStop(0, ac.base0); baseGrad.addColorStop(1, ac.base1);
    ctx.fillStyle = baseGrad; ctx.fill();
    ctx.strokeStyle = ac.outline; ctx.lineWidth = 2; ctx.stroke();
    ...
    if (ac.ringCount > 0 && ac.rings) {
        ...
        for (const p of positions) {
            ...
            ctx.fillStyle = ringG;
            ctx.beginPath(); ctx.roundRect(bx - 4, ry - rh / 2, bw + 8, rh, 3); ctx.fill();
            ctx.strokeStyle = ac.rings[0]; ctx.lineWidth = 1; ctx.stroke();
        }
    }
    ...
    ctx.beginPath(); ctx.roundRect(this.x - mw / 2, by - 6, mw, mh, 5); ctx.fill();
    ctx.strokeStyle = ac.outline; ctx.lineWidth = 1.5; ctx.stroke();

    // --- hit flash ---
    if (hitAlpha > 0) {
        ctx.globalAlpha = hitAlpha;
        ctx.fillStyle = '#ff2200';
        ctx.beginPath(); ctx.ellipse(this.x, this.y + 20, this.w / 2, 24, 0, 0, Math.PI * 2); ctx.fill();
        ctx.beginPath(); ctx.roundRect(bx - 4, by, bw + 8, bh, 6); ctx.fill();
        ctx.globalAlpha = 1;
    }
    ...
    if (this.muzzleFlash > 0) {
        ...
        ctx.fillStyle = flash;
        ctx.beginPath(); ctx.arc(this.x, by - 12, isFire ? 38 : 32, 0, Math.PI * 2); ctx.fill();
        ctx.globalAlpha = 1;
    }
...
Cannon.prototype.getCollector = function() {
